...
docList <- list(doc1, doc2, doc3, doc4, doc5) 
...
computeTFIDF <- function(row) { 
        ...
        w = rep(0, length(row)) 
        w[row > 0] = (1 + log2(row[row > 0])) * log2(nDocs/(df + 1)) 
    return(w) 
} 
...
# FunÃ§Ã£o scale aplicada na matriz 
n <- scale(n, center=FALSE, scale=sqrt(colSums(n^2))) 

# Ranqueamento via produto escalar (assumindo vetores jÃ¡ normalizados) 
query <- n[, nDocs + 1] 
n <- n[, 1:nDocs] 
